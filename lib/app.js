...
function list(req, res) {
  req.db.collection('project').find({$not:{archived: true}}, function (err, results) {
    if (err) return res.send('Failed to fetch projects')
      res.send(results)
  })
}
...
function get(req, res) {
  async.parallel({
    ...
    docs: function (next) {
      req.db.collection('document').find({hash: req.params.id}, function (err, docs) {
